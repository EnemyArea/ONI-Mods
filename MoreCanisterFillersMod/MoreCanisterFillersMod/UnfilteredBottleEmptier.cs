...
namespace MoreCanisterFillersMod
{
	...
	internal class UnfilteredBottleEmptier : StateMachineComponent<UnfilteredBottleEmptier.StatesInstance>, IEffectDescriptor
	{
		...
		public class StatesInstance : GameStateMachine<States, StatesInstance, UnfilteredBottleEmptier, object>.GameInstance
		{
			// Token: 0x0600004A RID: 74 RVA: 0x0000339C File Offset: 0x0000159C
			public StatesInstance(UnfilteredBottleEmptier smi) : base(smi)
			{
				this.Meter = new MeterController(this.GetComponent<KBatchedAnimController>(), "meter_target", "Meter", global::Meter.Offset.Infront, Grid.SceneLayer.NoLayer, new string[]
				{
					"meter_target",
					"meter_arrow",
					"meter_scale"
				});
				this.Subscribe(-1697596308, this.OnStorageChange);
			}
		...
		public class States : GameStateMachine<States, StatesInstance, UnfilteredBottleEmptier>
		{
			...
			public override void InitializeStates(out BaseState outDefaultState)
			{
				...
				StatusItem statusItem = new StatusItem("UnfilteredBottleEmptier", string.Empty, string.Empty, string.Empty, StatusItem.IconType.Info, NotificationType.Neutral, false, OverlayModes.None.ID);
				...
				this.root.ToggleStatusItem(this._statusItem, (StatesInstance smi) => smi.master);
				this.Unoperational.TagTransition(GameTags.Operational, this.Waitingfordelivery).PlayAnim("off");
				this.Waitingfordelivery.TagTransition(GameTags.Operational, this.Unoperational, true).EventTransition(GameHashes.OnStorageChange, this.Emptying, (StatesInstance smi) => !smi.GetComponent<Storage>().IsEmpty()).PlayAnim("on");
				this.Emptying.TagTransition(GameTags.Operational, this.Unoperational, true).EventTransition(GameHashes.OnStorageChange, this.Waitingfordelivery, (StatesInstance smi) => smi.GetComponent<Storage>().IsEmpty()).Update("Emit", delegate(StatesInstance smi, float dt)
				{
					smi.Emit(dt);
